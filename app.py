        ...
        if in_name and in_birth:
                ...
                if not result_p.empty:
                    ...
                    if not is_terminated:
                        ...
                        for index, row in result_p.iterrows():
                            ...
                            **무단결석:** {absent_date}  
                            **패널티:** {penalty_date}  
                            **횟수 산정:** {penalty_count}회
                            """)

                        with st.expander("🚨 무단결석 및 패널티 규정", expanded=False):
                            st.markdown("""
                            **무단결석 1회 → 패널티 봉사 2회 (익월 진행)**
                            * 패널티 2회 완수 시: 2회차 시간만 인정
                            * 패널티 미이행(결석) 시: 봉사시간 미지급 및 무단결석 횟수 추가
                            """)
            ...
            if found_something:
                if not is_terminated:
                    ...
                    **[상장 기준 안내]** 👀
                    * **6회 참여:** 감사장 / **8회 참여:** 공로상
                    * 활동 종료 시까지 중도 이탈(무단결석 3회 등)이 없어야 합니다.
                    """)
